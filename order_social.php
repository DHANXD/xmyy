...
$productCode = $s1['code'];
$productCategory = str_replace(array( "’","'" ),"&apos;",$s1['kategori']);
$productTitle = str_replace(array( "’","'" ),"&apos;",$s1['title']);
$jenisnya = $s1['jenis'];
...
$level = $s11['level'];
if($level == 'reseller'){
    $harga = $s1['harga_reseller']/1000;
}
else {
    $harga = $s1['harga_jual']/1000;
}

$total_bayar = round($harga*$qty);
$subtotal = $total_bayar - $potongan;
$subtotale = ($total_bayar - $potongan) + $kode_unik;
...
WA/TELEGRAM : '.$sellerWa;
        ...
        if($ct == 0){
            ...
            if($httpCode == 200){
                ...
                ('$last_id', '$reference', '$kd_transaksi', '$metode', '$metodeBayar', '$email', '$noHp', '$harga', '0', '$subtotal', '$vaNumber', '$paymentUrl', '$statusCode', '0000-00-00 00:00:00', '$expired_time', '$jenisnya', 3, '$created_date')") or die(mysqli_error($conn));
            }
...
WA/TELEGRAM : '.$sellerWa;
        ...
        if($metodeBayar == 'BCAVA'){
            ...
            $insert_tripay = mysqli_query($conn,"INSERT INTO `tb_tripay` 
                (`userID`, `reference`, `merchant_ref`, `payment_method`, `payment_name`, `customer_email`, `customer_phone`, `amount`, `fee`, `amount_total`, `pay_code`, `checkout_url`, `status`, `paid_time`, `expired_time`, `providerID`, `jenis_transaksi`, `created_date`) VALUES 
                ('$last_id', '', '$kd_transaksi', 'Direct_$metodeBayar', 'Bank Central Asia (BCA)', '$email', '$noHp', '$harga', '$kode_unik', '$subtotale', '', '', 'UNPAID', '0000-00-00 00:00:00', '0000-00-00 00:00:00', '$jenisnya', 3, '$created_date')") or die(mysqli_error($conn));
        }
            ...
            $insert_tripay = mysqli_query($conn,"INSERT INTO `tb_tripay` 
                (`userID`, `reference`, `merchant_ref`, `payment_method`, `payment_name`, `customer_email`, `customer_phone`, `amount`, `fee`, `amount_total`, `pay_code`, `checkout_url`, `status`, `paid_time`, `expired_time`, `providerID`, `jenis_transaksi`, `created_date`) VALUES 
                ('$last_id', '', '$kd_transaksi', 'Direct_$metodeBayar', 'Bank Negara Indonesia (BNI)', '$email', '$noHp', '$harga', '$kode_unik', '$subtotale', '', '', 'UNPAID', '0000-00-00 00:00:00', '0000-00-00 00:00:00', '$jenisnya', 3, '$created_date')") or die(mysqli_error($conn));
        }
            ...
            $insert_tripay = mysqli_query($conn,"INSERT INTO `tb_tripay` 
                (`userID`, `reference`, `merchant_ref`, `payment_method`, `payment_name`, `customer_email`, `customer_phone`, `amount`, `fee`, `amount_total`, `pay_code`, `checkout_url`, `status`, `paid_time`, `expired_time`, `providerID`, `jenis_transaksi`, `created_date`) VALUES 
                ('$last_id', '', '$kd_transaksi', 'Direct_$metodeBayar', 'Bank Rakyat Indonesia (BRI)', '$email', '$noHp', '$harga', '$kode_unik', '$subtotale', '', '', 'UNPAID', '0000-00-00 00:00:00', '0000-00-00 00:00:00', '$jenisnya', 3, '$created_date')") or die(mysqli_error($conn));
        }
            ...
            $insert_tripay = mysqli_query($conn,"INSERT INTO `tb_tripay` 
                (`userID`, `reference`, `merchant_ref`, `payment_method`, `payment_name`, `customer_email`, `customer_phone`, `amount`, `fee`, `amount_total`, `pay_code`, `checkout_url`, `status`, `paid_time`, `expired_time`, `providerID`, `jenis_transaksi`, `created_date`) VALUES 
                ('$last_id', '', '$kd_transaksi', 'Direct_$metodeBayar', 'Bank Mandiri (MANDIRI)', '$email', '$noHp', '$harga', '$kode_unik', '$subtotale', '', '', 'UNPAID', '0000-00-00 00:00:00', '0000-00-00 00:00:00', '$jenisnya', 3, '$created_date')") or die(mysqli_error($conn));
        }
        header('Location:../payment/?trxID='.$kd_transaksi);
        exit();
    }
        ...
        if($metode == 'saldo'){
            ...
            if($saldo_aktif > $subtotal){
                $insert = mysqli_query($conn,"INSERT INTO `tb_order` (`id_session`, `kd_transaksi`, `produkID`, `qty`, `harga`, `kode_unik`, `potongan`, `sub_total`, `voucher`, `kategori`, `title`, `userID`, `zoneID`, `nickname`, `trxID`, `services`, `status_order`, `note`, `full_name`, `email`, `no_hp`, `metode`, `created_date`, `providerID`, `jenis`, `id_user`, `status`) VALUES ('$sid', '$kd_transaksi', '$produkID', 1, '$potongan', '$subtotal', '$voucher', '$kode_unik', '$subtotal', '$productCategory', '$productTitle', '$userID', '$zone_id', '$nickname', '', '$productCode', '', '', '$full_name', '$email', '$noHp', '$metode', '$created_date', '$jenisnya', 3, '$last_id', 1)") or die(mysqli_error($conn));
                ...
                $insert_transaksi = mysqli_query($conn,"INSERT INTO `tb_transaksi` (`kd_transaksi`, `date`, `transaksi`, `total`, `saldo`, `note`, `providerID`, `jenis`, `metode`, `userID`, `status`) VALUES ('$kd_transaksi','$created_date','Pembelian Produk $productTitle','$subtotal',0,'Pembelian Produk $productTitle', '$jenisnya','3','saldo','$last_id',1)") or die(mysqli_error());
                ...
                if($jenisnya == 4){
                    ...
                    curl_setopt_array($curl1, array(
                        ...
                        CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                        CURLOPT_CUSTOMREQUEST => 'POST',
                        CURLOPT_POSTFIELDS => array('key' => $apiKeys, 'sign' => $sign, 'type' => 'order', 'service' => $productCode, 'quantity' => $qty, 'data_no' => $userID),
                    ));
                    ...
                    if($hasil['status'] == 'true'){
                        ...
                        $order_status = $hasil['data']['status'];
                        $update3 = mysqli_query($conn,"UPDATE `tb_order` SET `status` = 3, `status_order` = '$order_status', `note` = '$order_note' WHERE kd_transaksi = '$kd_transaksi'") or die(mysqli_error($conn));
                        $update = mysqli_query($conn,"UPDATE `tb_balance` SET `active` = active + '$subtotal', `pending` = pending - '$subtotal' WHERE userID = '$last_id'") or die(mysqli_error($conn));
                        $insert_transaksi = mysqli_query($conn,"INSERT INTO `tb_transaksi` (`kd_transaksi`, `date`, `transaksi`, `total`, `saldo`, `note`, `providerID`, `jenis`, `metode`, `userID`, `status`) VALUES ('$kd_transaksi','$created_date','Pengembalian Dana','$subtotal',0,'Pengembalian Dana','$jenisnya','1','saldo','$userID',2)") or die(mysqli_error());
                    }
                }
