                    ...
                    <div class="card-body">
                      <?php
                        ...
                        if(isset($_GET['postID'])){
                          $postID = $_GET['postID'];
                          $sql_2 = mysqli_query($conn,"SELECT * FROM `tb_tripayapi` WHERE id = '$postID'") or die(mysqli_error());
                          $s2 = mysqli_fetch_array($sql_2);
                        }
                      ?>
                      <form role="form" action="<?php echo $urlweb; ?>/function/add-tripay.php" method="post" enctype="multipart/form-data">
                        <div class="form-group mb-2">
                          <label class="form-label">Payment Gateway :</label>
                          <select class="form-control" type="text" name="provider" required>
                            <option value=""> Pilih Payment Gateway </option>
                            <option value="1"<?php if(isset($_GET['postID'])){ if($s2['id'] == 1){ echo ' selected=selected'; }} ?>> Tripay </option>
                            <option value="2"<?php if(isset($_GET['postID'])){ if($s2['id'] == 2){ echo ' selected=selected'; }} ?>> Ipaymu </option>
                            <option value="3"<?php if(isset($_GET['postID'])){ if($s2['id'] == 3){ echo ' selected=selected'; }} ?>> Duitku </option>
                          </select>
                        </div>
                        <div class="form-group mb-2">
                          <label class="form-label">Api Key / Virtual Account (Ipaymu) :</label>
                        ...
                        <div class="form-group mb-2">
                          <label class="form-label">Private / Secret / Signature Key :</label>
                          <input class="form-control" type="text" name="private_key" value="<?php echo $s2['private_key']; ?>">
                        </div>
                        <div class="form-group mb-2">
                          <label class="form-label">UserID / Merchant Code :</label>
                  ...
                  <div class="card">
                    <div class="card-datatable table-responsive">
                      <table id="default-datatable" class="invoice-list-table table border-top">
                        ...
                        <tbody>
                          <?php
                            $sql_1 = mysqli_query($conn,"SELECT * FROM `tb_tripayapi` WHERE jenis = 0 AND status = 1 ORDER BY id ASC") or die(mysqli_error());
                            $no=0;
                          ...
                          <tr>
                            ...
                            <td class="text-left" style="vertical-align: middle; white-space: normal; font-size: 14px;">
                              Api Key :<br><?php echo $s1['api_key']; ?><br>
                              Private / Secret / Signature Key :<br><?php echo $s1['private_key']; ?><br>
                              Userid / Merchant Code :<br><?php echo $s1['merchant_code']; ?><br>
