                  ...
                  <div class="card">
                    <div class="card-body">
                      <?php
                          ...
                          if ($_GET['notif'] == 1) {
                            echo '
                              <div class="alert alert-success d-flex align-items-center" role="alert">
                                ...
                                <span><strong>Well Done!</strong> Category Saved!</span>
                              </div>
                            ';
                          }
                          if ($_GET['notif'] == 2) {
                            echo '
                              <div class="alert alert-warning d-flex align-items-center" role="alert">
                                ...
                                <span><strong>Warning!</strong> Max Image Size 512KB!</span>
                              </div>
                            ';
                          }
                          if ($_GET['notif'] == 3) {
                        ...
                        if(isset($_GET['catID'])){
                          $catID = $_GET['catID'];
                          $sql_2 = mysqli_query($conn,"SELECT * FROM `tb_kategori` WHERE id = '$catID'") or die(mysqli_error());
                          $s2 = mysqli_fetch_array($sql_2);
                        }
                      ?>
                      <form role="form" action="<?php echo $urlweb; ?>/function/add-category.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group mb-2">
                          ...
                          <input class="form-control" type="text" name="kategori" value="<?php if(isset($_GET['catID'])) { echo $s2['kategori']; } ?>" required>
                          <input class="form-control" type="hidden" name="postID" value="<?php if(isset($_GET['catID'])) { echo $s2['id']; } ?>">
                        </div>
                        <div class="form-group mb-2">
                          <label class="form-label">SubTitle / Currency :</label>
                          <input class="form-control" type="text" name="subtitle" value="<?php if(isset($_GET['catID'])) { echo $s2['subtitle']; } ?>">
                        </div>
                        <div class="form-group mb-2">
                          <label class="form-label">Type :</label>
                          <select name="parent" class="form-control" required>
                            <option value=""> Pilih </option>
                            <option value="1"<?php if(isset($_GET['catID'])) { if($s2['parent'] == 1) { echo 'selected = selected'; }} ?>> Game</option>
                            <option value="2"<?php if(isset($_GET['catID'])) { if($s2['parent'] == 2) { echo 'selected = selected'; }} ?>> Akun Premium</option>
                            <option value="3"<?php if(isset($_GET['catID'])) { if($s2['parent'] == 3) { echo 'selected = selected'; }} ?>> Pulsa</option>
                            <option value="4"<?php if(isset($_GET['catID'])) { if($s2['parent'] == 4) { echo 'selected = selected'; }} ?>> E-Money</option>
                            <option value="5"<?php if(isset($_GET['catID'])) { if($s2['parent'] == 5) { echo 'selected = selected'; }} ?>> Social Media</option>
                          </select>
                        </div>
                        <div class="form-group mb-2">
                          <label class="form-label">Help Button :</label>
                          <textarea class="form-control summernoteEditor" type="text" name="bantuan"><?php if(isset($_GET['catID'])) { echo $s2['bantuan']; } ?></textarea>
                        </div>
                        <div class="form-group mb-2">
                        ...
                        <a href="<?php echo $urlweb; ?>/category/" class="btn btn-light">Cancel</a>
                      </form>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="card">
                    <div class="card-datatable table-responsive">
                      <table id="default-datatable" class="invoice-list-table table border-top">
                        ...
                        <tbody>
                          <?php
                            $sql_1 = mysqli_query($conn,"SELECT * FROM `tb_kategori` ORDER BY id ASC") or die(mysqli_error());
                            $no=0;
                          ...
                          <tr>
                            ...
                            <td class="text-center" style="vertical-align: middle; font-size: 14px;">
                              ...
                              <a href="<?php echo $urlweb; ?>/category/?catID=<?php echo $s1['id']; ?>" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
                              <a href="<?php echo $urlweb; ?>/function/del-category.php?id=<?php echo $s1['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure want remove this data?');"><i class="fa fa-trash"></i></a>
                            </td>
                          </tr>
                          <?php } ?>
                        </tbody>
