  ...
  <div id="wrapper">
            ...
            <div class="pb-3">
              <div class="section">
                <div class="card-body">
                  ...
                  <h5 style="margin-left: 45px; margin-top: 5px;">Input Link / Username</h5>
                  <div class="form-row pt-3">
                    <div class="col-6">
                      <input type="text" name="userID" id="userID" class="form-control mb-2" placeholder="Masukkan Link / Username" >
                    </div>
                    <div class="col-6">
                      <input type="number" class="form-control mb-2" name="qty" id="qty" value="100" step="1" min="100" placeholder="Jumlah">
                      <span>Minimal Pembelian 100</span>
                    </div>
                    <div class="col-12">
                      <select name="produkID" id="produkID" class="form-control mb-2">
                        <option value=""> Pilih Produk </option>
                        <?php
                          $sql_11 = mysqli_query($conn,"SELECT * FROM `tb_produk_social` WHERE kategori LIKE '$brand%' ORDER BY harga_jual ASC") or die(mysqli_error($conn));
                          while($s11 = mysqli_fetch_array($sql_11)){
                        ?>
                        <option value="<?php echo $s11['id']; ?>"> <?php echo $s11['title']; ?> </option>
                        <?php } ?>
                      </select>
                    </div>
                    <div class="row p-1" id="hasilsocial"></div>
    ...
    <script>
      ...
      $(document).ready(function() {
        $("#produkID").change(function (){
            url = "<?php echo $urlweb; ?>/select_social.php?id="+$(this).val()+"&qty="+$("#qty").val();
            $('#accordionExample').load(url);
            //console.log(url);
            return false;
        });
        $("#qty").keyup(function(){
            url = "<?php echo $urlweb; ?>/select_sociale.php?id="+$(this).val()+"&produkID="+$("#produkID").val();
            $('#accordionExample').load(url);
            //console.log(url);
            return false;
        });
        ...
        $("#submitId").click(function () {
            uri = "<?php echo $urlweb; ?>/get_social.php?userID="+$("#userID").val()+"&produkID="+$('#produkID').val()+"&metode="+$('input:radio[name=metode]:checked').val()+"&IDuser="+$("#IDuser").val()+"&noHp="+$("#noHp").val()+"&qty="+$("#qty").val()+"&voucher="+$("#voucher").val();
